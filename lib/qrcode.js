  ...
  app.get('/issue/:license/:code', function(req, res){
    ...
    case 'C':
      ...
      if(result.length > 0) {
        ...
        return lib.response(req, res, 200, {code: code, data: result[0]});
      }
      else {
        return lib.response(req, res, 404, {code: code});
      }
      break;
    case 'Q':
    default :
      if((ticket = lib.mysql.searchTicketByCode([code])) == null) return lib.response(req, res, 208,  {qrcode: code, license: license});
      var result = lib.mysql.updTicketByCode ([license, pos.rcn, member.name, member.bzname, icon.icon_path, code]);
