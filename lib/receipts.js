...
module.exports = function(app) {
  ...
  app.post('/receipt/probe/:license', function (req, res) {
    ...
    lib.parser (dat, function (err, parsed) {
      ...
      lib.mysql.putUserDealHistory ([ticket.email, ticket.member, ticket.rcn, "", "거래", "현금", lib.utils.toInt(parsed.total)]); 
      if (coupons != null) {
        coupons.forEach(coupon => {
          //console.log(moment(coupon.date2).format('YYYY-MM-DD HH:mm:ss'),moment(coupon.date2).unix());
          if(coupon.status == '사용' && moment().unix() <= moment(coupon.date2).unix()) {
            switch(coupon.ctype) {
            case '프로모션': 
            ...
            case '리워드'  : 
              ...
              break;
            case '스탬프'  : 
              if(coupon.overagain == '가능') {
                n_stamp  = parseInt(lib.utils.toInt(parsed.total)/coupon.limits); 
              }
              for(var i = 0; i < n_stamp; i++) {
                lib.mysql.generateUserStamp ([coupon.id, ticket.email, ticket.rcn]);
              }
              lib.mysql.putUserDealHistory ([ticket.email, ticket.member, ticket.rcn, "", "스탬프", "적립", n_stamp]); 
              break;
            }
          }
        });
      }
      var record = [user.email, ticket.member, ticket.rcn, ticket.bzname, ticket.phone, ticket.icon, `/txt/${key}.txt`, `/pdf/${key}.pdf`, '현금', lib.utils.toInt(parsed.total), n_promotion, n_reward, n_stamp, c_promotion, c_reward, cpurl, registered];
  ...
  app.post('/publish/event/coupon/:cid', function(req, res){
    var cid      = req.params.cid;
    var email    = req.body.email;
    var timstamp = moment().format('YYYY-MM-DD HH:mm:ss');

    var coupon = lib.mysql.searchMemberCouponById ([cid]);
    console.log (coupon);
    var member = lib.mysql.searchMemberByRcn ([coupon.rcn]);
    console.log (member);

    var uid = lib.mysql.getTableAutoInc(['user_coupon']);
    lib.mysql.generateUserCoupon ([email, coupon.member, coupon.rcn, coupon.bzcode, member.phone, member.opentime, member.closetime, member.address, coupon.ctype, `${coupon.cpcode}-${uid}`, coupon.name, coupon.date1, coupon.date2, coupon.benefit, coupon.notice, member.lat, member.lng]);
  ...
  app.post('/publish/coupon/promotion/:rcn/:cid', function(req, res){
...
console.log(email);
    ...
    // TODO: bzcode -> bzname
    var uid = lib.mysql.getTableAutoInc(['user_coupon']);
    lib.mysql.generateUserCoupon ([email, member.name, member.rcn, coupon.bzcode, member.phone, member.opentime, member.closetime, member.address, coupon.ctype, `${coupon.cpcode}-${uid}`, coupon.name, coupon.date1, coupon.date2, coupon.benefit, coupon.notice, member.lat, member.lng]);
    lib.response(req, res, 200);
  });
  ...
  app.post('/publish/coupon/:rcn/:id/:cid', function(req, res){
    ...
    case '리워드'  : 
      ...
      lib.mysql.putUserDealHistory ([receipt.email, receipt.member, receipt.rcn, "", "리워드", "적립", 1]); 
    ...
    case '프로모션': 
      ...
      lib.mysql.putUserDealHistory ([receipt.email, receipt.member, receipt.rcn, "", "프로모션", "적립", 1]); 
      lib.mysql.incPromotionCouponById ([cid]);
    ...
    // TODO: bzcode -> bzname
    var uid = lib.mysql.getTableAutoInc(['user_coupon']);
    lib.mysql.generateUserCoupon ([receipt.email, receipt.member, receipt.rcn, coupon.bzcode, member.phone, member.opentime, member.closetime, member.address, coupon.ctype, `${coupon.cpcode}-${uid}`, coupon.name, coupon.date1, coupon.date2, coupon.benefit, coupon.notice, member.lat, member.lng]);
    lib.response(req, res, 200);
  });
