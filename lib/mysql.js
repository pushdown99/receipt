...
console.log (record);
  ...
  searchCouponHistoryRcn: function (record) {
    return this.query("SELECT * FROM admin_coupon_history WHERE rcn = ?", record);
  },
...
console.log (record);
  ...
  searchAdminUserReceipt: function (record) {
    return this.query("SELECT * FROM receipts WHERE (member like ? AND rcn like ?) AND registered >= ? AND registered <= ? AND email = ? ORDER BY updated DESC", record);
  },
