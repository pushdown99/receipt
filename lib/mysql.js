...
module.exports = {
  ...
  generateUserStamp: function(record) {
    ...
    if(stamps.length <= 0) {
      ...
      var record = [email, coupon.member, coupon.rcn, coupon.bzcode, member.phone, member.opentime, member.closetime, member.address, coupon.ctype, `${coupon.cpcode}-${uid}`, coupon.name, coupon.overagain, coupon.stamp, coupon.date1, coupon.date2, coupon.benefit, coupon.notice, member.lat, member.lng];
      var result = this.query ("INSERT INTO user_stamp (email, member, rcn, bzcode, phone, opentime, closetime, address, ctype, cpcode, name, overagain, total, date1, date2, benefit, notice, lat, lng) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", record);
  ...
  searchMemberNear: function (lat, lng, distance = 2.0) {
    return this.query ("SELECT *, (6371*acos(cos(radians(?))*cos(radians(lat))*cos(radians(lng)-radians(?))+sin(radians(?))*sin(radians(lat)))) AS distance FROM admin_member WHERE exposure='노출' HAVING distance <= ? ORDER BY distance LIMIT 0,300", [lat, lng, lat, distance]);
  },
  searchMemberByRcn: function (record) {
    return (this.query ("SELECT * FROM admin_member WHERE rcn = ?", record))[0];
  },
...
console.log (record);
  ...
  searchStatMembersSparkline: function (record) {
    return (this.query("SELECT * FROM (SELECT DATE_FORMAT(registered,'%Y-%m-%d') day, IFNULL(sum(if(status= '가입', 1, 0)),0) regi, IFNULL(sum(if(status= '해지', 1, 0)),0) term FROM admin_member WHERE DATE_FORMAT(registered,'%Y-%m-%d') <= '2021-02-10' GROUP BY day ORDER BY day DESC limit 10) as a order by day", record));
  },
  ...
  searchMemberHistory: function (record) {
    return this.query("SELECT * FROM admin_member_history ORDER BY updated DESC", record);
  },
  searchMemberHistoryByRcn: function (record) {
    return this.query("SELECT * FROM admin_member_history WHERE rcn = ? ORDER BY updated DESC", record);
  },
  searchEventHistory: function (record) {
    return this.query("SELECT * FROM admin_event_history ORDER BY updated DESC", record);
  },
  searchCouponHistory: function (record) {
    return this.query("SELECT * FROM admin_coupon_history ORDER BY updated DESC", record);
  },
  searchCouponHistoryRcn: function (record) {
    return this.query("SELECT * FROM admin_coupon_history WHERE rcn = ? ORDER BY updated DESC", record);
  },
  searchNoticeHistory: function (record) {
    return this.query("SELECT * FROM admin_notice_history ORDER BY updated DESC", record);
  },
  searchAdminHistory: function (record) {
    return this.query("SELECT * FROM admin_admin_history ORDER BY updated DESC", record);
  },
  searchClassHistory: function (record) {
    return this.query("SELECT * FROM admin_class_history ORDER BY updated DESC", record);
  },
  searchUserHistory: function (record) {
    return this.query("SELECT * FROM admin_users_history ORDER BY updated DESC", record);
  },
  searchGroupHistory: function (record) {
    return this.query("SELECT * FROM admin_group_history ORDER BY updated DESC", record);
  },
  ...
  searchAdminGenderDay: function (record) {
    return this.query("SELECT DATE_FORMAT(registered,'%Y-%m-%d') day, sum(if(gender= '남' AND status='가입', 1, 0)) male, sum(if(gender= '여' AND status='가입', 1, 0)) female, sum(if(status='가입',1,0)) regi, sum(if(status='해지',1,0)) term FROM users WHERE (registered >= ? AND registered <= ?) OR (termination >= ? AND termination <= ?) GROUP BY day ORDER BY day DESC", record);
  },
...
console.log (record);
    return this.query("SELECT DATE_FORMAT(registered,'%Y-%m-%d') day, SUM( IF( ((YEAR(CURDATE())-birth_year) DIV 10 * 10)=10,1,0 )) g10, SUM( IF( ((YEAR(CURDATE())-birth_year) DIV 10 * 10)=20,1,0 )) g20, SUM( IF( ((YEAR(CURDATE())-birth_year) DIV 10 * 10)=30,1,0 )) g30, SUM( IF( ((YEAR(CURDATE())-birth_year) DIV 10 * 10)=40,1,0 )) g40, SUM( IF( ((YEAR(CURDATE())-birth_year) DIV 10 * 10)=50,1,0 )) g50, SUM( IF( ((YEAR(CURDATE())-birth_year) DIV 10 * 10)=60,1,0 )) g60, SUM( IF( ((YEAR(CURDATE())-birth_year) DIV 10 * 10)=70,1,0 )) g70, COUNT(*) total FROM users  WHERE registered >= ? AND registered <= ? GROUP BY day ORDER BY day DESC", record);
  },
...
console.log (record);
  ...
  searchAdminMember: function (record) {
    return this.query("SELECT * FROM admin_member WHERE (name like ? OR rcn like ?) AND status like ? AND area1 like ? AND area2 like ? AND updated >= ? AND updated <= ? ORDER BY updated DESC", record);
  },
  searchAdminUser: function (record) {
    return this.query("SELECT *, ((YEAR(CURDATE())-birth_year) DIV 10 * 10) as age FROM users WHERE email like ? AND os like ? AND gender like ? AND ((YEAR(CURDATE())-IFNULL(birth_year,'1900')) DIV 10 * 10) like ? AND area1 like ? AND area2 like ? AND registered >= ? AND registered <= ? ORDER BY updated DESC", record);
  },
  searchAdminUserReceipt: function (record) {
    return this.query("SELECT * FROM receipts WHERE (member like ? AND rcn like ?) AND registered >= ? AND registered <= ? AND email = ? ORDER BY registered DESC", record);
  },

  searchAdminUserCoupon: function (record) {
    return this.query("SELECT * FROM user_coupon WHERE (member like ? AND rcn like ?) AND registered >= ? AND registered <= ? AND email = ? ORDER BY updated DESC", record);
  },

  searchAdminUserStamp: function (record) {
    return this.query("SELECT * FROM user_stamp WHERE (member like ? AND rcn like ?) AND registered >= ? AND registered <= ? AND email = ? ORDER BY uodated DESC", record);
  },

  searchAdminCoupon: function (record) {
    return this.query("SELECT * FROM admin_member_coupon WHERE name like ? AND status like ? AND updated >= ? AND updated <= ? ORDER BY updated DESC", record);
  },
  searchAdminEvent: function (record) {
    return this.query("SELECT * FROM admin_event WHERE status like ? AND coupon like ? AND updated >= ? AND updated <= ? ORDER BY updated DESC", record);
  },
  searchAdminNotice: function (record) {
    return this.query("SELECT * FROM admin_notice WHERE title like ? AND gender like ? AND age like ? AND area1 like ? AND area2 like ? AND updated >= ? AND updated <= ? ORDER BY updated DESC", record);
  },
  ...
  searchAdminClass: function (record) {
    return this.query("SELECT * FROM admin_class WHERE name like ? AND updated >= ? AND updated <= ? ORDER BY updated DESC", record);
  },


/*
  searchMemberDashDealDay: function (record) {
    return this.query("SELECT DATE_FORMAT(registered, '%Y-%m-%d') day, DATE_FORMAT(registered, '%Y-%m-%d') pattern, sum(amount) total, max(id) maxid, min(id) minid FROM receipts WHERE rcn = ? AND registered >= ? AND registered <= ? GROUP BY DATE(registered) ORDER BY registered DESC", record);
  },
  searchMemberDashDealWeek: function (record) {
    return this.query("SELECT DATE_FORMAT(registered, '%Y-%m-%d') day, DATE_FORMAT(registered,'%X %V') pattern, sum(amount) total, max(id) maxid, min(id) minid FROM receipts WHERE rcn = ? AND registered >= ? AND registered <= ? GROUP BY pattern ORDER BY registered DESC", record);
  },
  searchMemberDashDealMon: function (record) {
    return this.query("SELECT DATE_FORMAT(registered, '%Y-%m') day, DATE_FORMAT(registered,'%X %c') pattern, sum(amount) total, max(id) maxid, min(id) minid FROM receipts WHERE rcn = ? AND registered >= ? AND registered <= ? GROUP BY pattern ORDER BY registered DESC", record);
  },
*/
...
*/
  ...
  putAdminCouponHistory: function (record) {
    return this.query("INSERT INTO admin_coupon_history (rcn, name, menu, updater, updated, done, division, description) VALUES (?,?,?,?,?,?,?,?)", record);
  },
  ...
  updAdminMember: function (record) {
    return this.query("UPDATE admin_member SET status = ?, exposure = ?, name = ? , owner = ? , bzcond = ? , bztype = ? , bzname = ? , phone = ? , opening = ? , area1 = ? , area2 = ? , address = ? , lat = ? , lng = ?, updater = ?  WHERE id = ?", record);
  },
...
console.log (record);
  ...
  delAdminUserId: function (record) {
    return this.query("DELETE FROM users WHERE id =?", record);
  },
  delAdminUserEmail: function (record) {
    this.query("DELETE FROM users WHERE email =?", record);
    this.query("DELETE FROM user_stamp WHERE email =?", record);
    this.query("DELETE FROM user_coupon WHERE email =?", record);
    return this.query("DELETE FROM receipts WHERE email =?", record);
  },
  ...
  delMemberLeaflet: function (record) {
    return this.query("UPDATE admin_member SET leaflet = NULL WHERE rcn = ?", record);
  },
  ...
  getUserSearch: function (record) {
    return this.query("SELECT * FROM users WHERE (name like ? OR rcn like ?) AND status like ? AND area1 like ? AND area2 like ? AND updated >= ? AND updated <= ? ORDER BY id DESC", record);
  },
  ...
  updUserAreaEmail: function (record) {
    return this.query("UPDATE users SET area1 = ?, area2 = ? WHERE email = ?", record);
  },
  ...
  getMemberAll: function () {
    return this.query("SELECT * FROM admin_member ORDER BY name DESC");
  },
  ...
  getAdminAdmin: function (record) {
    return this.query("SELECT * FROM admin_users WHERE name like ? AND grade like ? AND status like ? AND updated >= ? AND updated <= ? ORDER BY updated DESC", record);
  },
  ...
  putAdminGroup: function (record) {
    console.log (record);
    return this.query("INSERT INTO admin_group (gname, pattern, gtype, gdepth, name, fcoupon, fevent, fnotice, fadmin, fgroup, homepage, register) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", record);
  },
  ...
  getAdminEvent: function (record) {
    return this.query("SELECT * FROM admin_event WHERE title like ? AND gender like ? AND age like ? AND area1 like ? AND area2 like ? AND updated >= ? AND updated <= ? ORDER BY updated DESC", record);
  },
  ...
  getAdminGroup: function (record) {
    return this.query("SELECT * FROM admin_group WHERE name like ? AND updated >= ? AND updated <= ? AND pattern like ? ORDER BY updated DESC", record);
  },

  getAdminLicense: function (record) {
    return this.query("SELECT * FROM pos WHERE (name like ? OR rcn like ?) AND status like ? AND license like ? AND updated >= ? AND updated <= ? ORDER BY updated DESC", record);
  },

  getAdminMonitor: function (record) {
    return this.query("SELECT * FROM pos WHERE (name like ? OR rcn like ?) AND version like ? AND status like ? AND network like ? ORDER BY updated DESC", record);
  },
  ...
  getEvent: function(record = null) {
    return this.query("SELECT * FROM admin_event ORDER BY id DESC");
  },
  ...
  getAdminEventBanner: function () {
    return this.query("SELECT * FROM admin_event WHERE fevent = 1 ORDER BY id desc");
  },

  getAdminMainBanner: function () {
    return this.query("SELECT * FROM admin_event WHERE fmain = 1 ORDER BY id desc");
  },
