...
module.exports = {
  ...
  putAdminEvent: function (record) {
    return this.query("INSERT INTO admin_event (title, status, fnotice, fweight, fmain, fevent, gender, age, area1, area2, date1, date2, event, main, detail, rgb, rgb2, coupon, date3, date4, coupon_url) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ? ,?, ?, ?, ?, ?)", record);
  },
...
console.log(record);
  ...
  searchMemberStamp: function (record) {
    return this.query("SELECT * from admin_member_coupon WHERE rcn = ? AND ctype = '스탬프'", record);
  },
  ...
  updAdminMember: function (record) {
    return this.query("UPDATE admin_member set name = ? , owner = ? , bzcond = ? , bztype = ? , bzname = ? , phone = ? , opening = ? , area1 = ? , area2 = ? , address = ? , lat = ? , lng = ? WHERE id = ?", record);
  },

  updAdminUser: function (record) {
    return this.query("UPDATE users set birth_year = ? , birth_month = ? , birth_day = ? , gender = ? , area1 = ? , area2 = ? , status = ? WHERE id = ?", record);
  },

  updAdminCoupon: function (record) {
    return this.query("UPDATE admin_member_coupon set name = ? , status = ? , date1 = ? , date2 = ? , benefit = ? , notice = ? WHERE id = ?", record);
  },

  updAdminEvent: function (record) {
    return this.query("UPDATE admin_event set title = ? , status = ?, fnotice = ?, fweight = ?, fmain = ?, fevent = ?, gender = ? , age = ? , area1 = ?, area2 = ?, date1 = ?, date2 = ? , event = ? , main = ?, detail = ?, rgb = ?, rgb2 = ?, coupon = ?, date3 = ?, date4 = ?, coupon_url = ? WHERE id = ?", record);
  },
