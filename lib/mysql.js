...
module.exports = {
  ...
  updTicketByCode: function(record) {
    return this.query("UPDATE ticket SET license = ?, rcn = ?, member = ?, bzname = ?, icon = ? WHERE qrcode = ?", record);
  },
  ...
  generateUserStamp: function(record) {
    ...
    if(stamps.length <= 0) {
      var uid = lib.mysql.getTableAutoInc(['user_stamp']);
      var record = [email, coupon.member, coupon.rcn, coupon.bzcode, coupon.ctype, `${coupon.cpcode}-${uid}`, coupon.name, coupon.stamp, coupon.date1, coupon.date2, coupon.benefit, coupon.notice, member.lat, member.lng];
      var result = this.query ("INSERT INTO user_stamp (email, member, rcn, bzcode, ctype, cpcode, name, total, date1, date2, benefit, notice, lat, lng) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", record);
  ...
  chgUsetStampCoupon: function (record) {
    ...
    var result = lib.mysql.generateUserCoupon ([stamp.email, stamp.member, stamp.rcn, stamp.bzcode, stamp.ctype, `${stamp.cpcode}-${uid}`, stamp.name, stamp.date1, stamp.date2, stamp.benefit, stamp.notice, stamp.lat, stamp.lng]);
    this.query("UPDATE user_stamp SET status = '사용' WHERE id = ?", record);
    return result;
  },
  generateUserCoupon: function(record) {
    console.log (record);
    return this.query("INSERT INTO user_coupon (email, member, rcn, bzcode, ctype, cpcode, name, date1, date2, benefit, notice, lat, lng) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", record);
  },
  ...
  putReceipts: function(record) {
    return this.query("INSERT INTO receipts (email, member, rcn, bzname, icon, text, pdf, payment, amount, n_promotion, n_reward, n_stamp, c_promotion, c_reward, cpurl, registered) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)", record);
  },
...
console.log (record);
  ...
  searchAdminMember: function (record) {
    return this.query("SELECT * FROM admin_member WHERE (name like ? OR rcn like ?) AND status like ? AND area1 like ? AND area2 like ? AND updated >= ? AND updated <= ? ORDER BY id DESC", record);
  },
  searchAdminUser: function (record) {
    return this.query("SELECT *, ((YEAR(CURDATE())-birth_year) DIV 10 * 10) as age FROM users WHERE email like ? AND os like ? AND gender like ? AND ((YEAR(CURDATE())-IFNULL(birth_year,'1900')) DIV 10 * 10) like ? AND area1 like ? AND area2 like ? AND registered >= ? AND registered <= ? ORDER BY id DESC", record);
  },
...
console.log (record);
  ...
  putAdminGroup: function (record) {
    console.log (record);
    return this.query("INSERT INTO admin_group (gname, name, fcoupon, fevent, fnotice, fadmin, fgroup, homepage, register) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)", record);
  },
  ...
  getAdminGroup: function (record) {
    return this.query("SELECT * FROM admin_group WHERE name like ? AND updated >= ? AND updated <= ?", record);
  },
